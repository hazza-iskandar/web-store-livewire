...
<div class="relative group overflow-hidden min-h-90 shadow rounded-lg">
    <a href="{{ route('products.show', $product->slug) }}" class="max-w-sm bg-white rounded-lg" wire:navigate>
        <div class="overflow-hidden h-50">
            @if ($product->created_at->between(today(), today()->addDays(5)))
                {{--  ambil hari ini dan 5 hari kedepan --}}
                <span
